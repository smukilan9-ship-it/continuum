  ...
  return (
    <div className="screen">
      ...
      <section className="research-layout">
        ...
        <Card className="source-library"><div className="library-heading"><div><p className="eyebrow">SOURCE LIBRARY</p><h2>{state.sources.length} indexed source{state.sources.length === 1 ? "" : "s"}</h2></div><Database size={20} /></div>{state.sources.map((source) => <div className="source-row" key={text(source, "id")}><FileText size={18} /><div><strong>{text(source, "title")}</strong><span>{text(source, "mimeType", "document")} · v{String(source.sourceVersion ?? 1)}</span><small>Indexed {formatDate(source.createdAt)}</small></div><button disabled={busy} onClick={() => void deleteSource(text(source, "id"), text(source, "title"))} aria-label={`Delete ${text(source, "title")}`}><Trash2 size={16} /></button></div>)}{!state.sources.length ? <div className="empty-inline"><FileText size={19} /><span>Add a PDF or text file to create stable, source-linked passages.</span></div> : null}</Card>
      </section>

      {state.decisions.length ? <section className="decision-ledger"><div className="section-heading"><div><p className="eyebrow">DECISION LEDGER</p><h2>Accepted and superseded choices</h2></div></div>{state.decisions.map((decision) => <Card className="decision-row" key={text(decision, "id")}><ShieldCheck size={18} /><div><strong>{text(decision, "text")}</strong><p>{text(decision, "reasoning")}</p><span>{list(decision, "sourceIds").length} linked sources · {formatDate(decision.createdAt)}</span></div><Badge tone={text(decision, "status") === "accepted" ? "green" : "neutral"}>{text(decision, "status", "accepted")}</Badge></Card>)}</section> : null}
    </div>
  );
}
