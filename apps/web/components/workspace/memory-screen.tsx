...
export function MemoryScreen({ state, showToast }: { state: WorkspaceState; showToast: Toast }) {
  ...
  return (
    <div className="screen">
      ...
      {mastery ? <Card className="mastery-summary"><div className="card-heading-row"><div><p className="eyebrow">CURRENT LEARNING STATE</p><h2>{conceptLabel(text(mastery, "conceptId", ""))}</h2><p>{text(mastery, "explanation")}</p></div><Badge tone={statusTone(text(mastery, "status", "not_started"))}>{masteryLabel(text(mastery, "status", "not_started"))}</Badge></div><div className="mastery-bars">{([{ label: "Exposure", key: "exposure" }, { label: "Understanding", key: "understanding" }, { label: "Transfer", key: "transfer" }, { label: "Retention", key: "retention" }]).map((metric) => <div key={metric.key}><div><span>{metric.label}</span><strong>{Math.round(number(mastery, metric.key) * 100)}%</strong></div><Progress value={number(mastery, metric.key) * 100} label={`${metric.label} mastery`} /></div>)}</div><small>{list(mastery, "evidenceIds").length} evidence item{list(mastery, "evidenceIds").length === 1 ? "" : "s"} linked</small></Card> : null}

      {results ? <section className="memory-section"><div className="section-heading"><div><p className="eyebrow">SEARCH RESULTS</p><h2>{results.length} relevant record{results.length === 1 ? "" : "s"}</h2></div><button onClick={() => setResults(undefined)}>Clear search</button></div><div className="memory-list">{results.map((result) => <Card className="memory-row" key={text(result, "id")}><div><Badge tone="blue">{formatLabel(text(result, "kind", "memory"))}</Badge><h3>{text(result, "content")}</h3><p>{formatDate(result.occurredAt)} · {Math.round(number(result, "tokenEstimate"))} estimated tokens</p></div>{typeof result.score === "number" ? <span>{Math.round(number(result, "score") * 100)}% match</span> : null}</Card>)}{!results.length ? <Card className="empty-record"><Search size={23} /><h2>No relevant memory found</h2><p>Try a broader phrase. Continuum does not return unrelated history just to fill the screen.</p></Card> : null}</div></section> : null}
      ...
      <section className="memory-section"><div className="section-heading"><div><p className="eyebrow">RECENT DURABLE EVENTS</p><h2>Where the current state came from</h2></div></div><div className="memory-list">{state.events.slice(0, 30).map((event) => <Card className="memory-row" key={text(event, "id")}><div><Badge tone="neutral">{eventTypeLabel(text(event, "type"))}</Badge><h3>{text(event, "summary")}</h3><p>{formatDate(event.occurredAt)}</p></div></Card>)}</div></section>
    </div>
  );
}
