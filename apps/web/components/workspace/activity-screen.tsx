  ...
  return (
    <div className="screen">
      ...
      <div className="activity-ledger-layout">
        {state.modelRoutes.length ? <section className="activity-section activity-ai-ledger"><div className="section-heading"><div><p className="eyebrow">AI ASSISTANCE</p><h2>Why a cloud route was used</h2></div></div><div>{state.modelRoutes.slice(0, 20).map((route) => <article key={text(route, "id")}><div><Badge tone="blue">{formatLabel(text(route, "taskClass", "assistance"))}</Badge><time>{formatDate(route.createdAt)}</time></div><h3>{text(route, "reason", "Selected by task capability, reliability, context, and cost policy.")}</h3><p>Provider choice and fallback use are retained in the audit record.</p></article>)}</div></section> : null}

        <section className="activity-section activity-event-ledger"><div className="section-heading"><div><p className="eyebrow">AUDIT TRAIL</p><h2>Recent durable events</h2></div></div><div>{state.events.slice(0, 20).map((event) => <article key={text(event, "id")}><i /><div><Badge tone="neutral">{eventTypeLabel(text(event, "type", "event"))}</Badge><h3>{text(event, "summary")}</h3><time>{formatDate(event.occurredAt)}</time></div></article>)}{!state.events.length ? <div className="review-empty-state"><Clock3 size={23} /><div><h3>No durable event yet</h3><p>Completed work, accepted decisions, learning evidence, and assistant updates will appear here with provenance.</p></div></div> : null}</div></section>
      </div>
    </div>
  );
}
