...
const PlanPage = dynamic(() => import("./plan/plan-page").then((module) => module.PlanPage), { loading });
...
const StudyView = dynamic(() => import("./study/study-view").then((module) => module.StudyView), { loading });
const BuildScreen = dynamic(() => import("./build/build-screen").then((module) => module.BuildScreen), { loading });
...
export function WorkspaceScreens({ view, state, user, userName, serverNow, goalId, projectId, shellGoals, onNavigate, onRefresh, showToast }: { view: WorkspaceView; state: WorkspaceState; user: AuthUser; userName: string; serverNow: string; goalId?: string; projectId?: string; shellGoals: Array<{ id: string; title: string; progress: number; targetDate: string; status: string }>; onNavigate: (view: WorkspaceView) => void; onRefresh: () => Promise<void>; showToast: Toast }) {
  ...
  if (view === "goals") return <PlanPage state={state} timeZone={user.timezone} serverNow={serverNow} showToast={showToast} onRefresh={onRefresh} />;
  if (view === "learn") return <StudyView state={state} showToast={showToast} onRefresh={onRefresh} />;
  if (view === "code") return <BuildScreen state={state} user={user} showToast={showToast} />;
