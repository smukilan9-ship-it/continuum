...
export function WorkspaceScreens({ view, state, user, userName, onNavigate, onRefresh, showToast }: { view: WorkspaceView; state: WorkspaceState; user: AuthUser; userName: string; onNavigate: (view: WorkspaceView) => void; onRefresh: () => Promise<void>; showToast: Toast }) {
  if (view === "today") return <TodayScreen state={state} userName={userName} onNavigate={onNavigate} onRefresh={onRefresh} />;
  if (view === "goals") return <GoalsScreen state={state} showToast={showToast} onRefresh={onRefresh} />;
  if (view === "learn") return <LearnScreen state={state} showToast={showToast} onRefresh={onRefresh} />;
  ...
  if (view === "activity") return <ActivityScreen state={state} showToast={showToast} onRefresh={onRefresh} />;
  return null;
}
