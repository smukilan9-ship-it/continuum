...
export const continuumTools: ContinuumTool[] = [
  ...
  tool({ name: "list_context_packs", title: "List compact context packs", description: "Lists private week, misconception, goal, and project packs with stable IDs, provenance, freshness, record counts, and token estimates; it does not return pack contents.", requiredScope: "memory:read", class: "read", inputSchema: z.object({}), inputJsonSchema: { type: "object", properties: {} } }),
  tool({ name: "get_context_pack", title: "Get one compact context pack", description: "Returns one user-owned pack by stable ID within a strict token budget, including provenance and privacy metadata. Unrelated account history is omitted.", requiredScope: "memory:read", class: "read", inputSchema: z.object({ packId: z.string().min(3).max(300), maxTokens: z.number().int().min(200).max(4000).default(1800) }), inputJsonSchema: { type: "object", required: ["packId"], properties: { packId: { type: "string" }, maxTokens: { type: "integer", minimum: 200, maximum: 4000, default: 1800 } } } }),
  tool({ name: "get_context_changes_since", title: "Get context changes since a timestamp", description: "Returns a compact, freshness-stamped delta of durable state changes after an ISO timestamp instead of retransmitting the full account.", requiredScope: "memory:read", class: "read", inputSchema: z.object({ since: z.string().datetime({ offset: true }), limit: z.number().int().min(1).max(100).default(50), maxTokens: z.number().int().min(200).max(4000).default(1200) }), inputJsonSchema: { type: "object", required: ["since"], properties: { since: { type: "string", format: "date-time" }, limit: { type: "integer", minimum: 1, maximum: 100 }, maxTokens: { type: "integer", minimum: 200, maximum: 4000 } } } }),
  ...
  tool({ name: "record_approved_update", title: "Record an explicitly approved update", description: "Appends an approved note or progress fact with stable entity ID and provenance. It cannot complete tasks, change schedules, or accept research decisions.", requiredScope: "memory:write", class: "write", inputSchema: z.object({ kind: z.enum(["note", "progress"]), entityId: z.string().min(3), summary: z.string().min(3).max(1000), detail: z.string().min(1).max(10_000), goalId: z.string().optional(), projectId: z.string().optional(), provenance: z.array(z.string().min(1).max(500)).min(1).max(30), approval: z.object({ approvedBy: z.string().min(1).max(200), approvedAt: z.string().datetime({ offset: true }) }) }), inputJsonSchema: { type: "object", required: ["kind", "entityId", "summary", "detail", "provenance", "approval"], properties: { kind: { type: "string", enum: ["note", "progress"] }, entityId: { type: "string" }, summary: { type: "string" }, detail: { type: "string" }, goalId: { type: "string" }, projectId: { type: "string" }, provenance: { type: "array", minItems: 1, maxItems: 30, items: { type: "string" } }, approval: { type: "object", required: ["approvedBy", "approvedAt"], properties: { approvedBy: { type: "string" }, approvedAt: { type: "string", format: "date-time" } } } } }, confirmationRequired: true }),
  ...
  tool({ name: "commit_schedule_change", title: "Commit confirmed internal schedule change", description: "Commits a reviewed Continuum schedule proposal only with schedule:commit and explicit confirmation metadata.", requiredScope: "schedule:commit", class: "write", inputSchema: z.object({ proposalId: z.string().min(3), confirmation: z.object({ confirmedBy: z.string().min(1), confirmedAt: z.string().datetime({ offset: true }) }) }), inputJsonSchema: { type: "object", required: ["proposalId", "confirmation"], properties: { proposalId: { type: "string" }, confirmation: { type: "object", required: ["confirmedBy", "confirmedAt"], properties: { confirmedBy: { type: "string" }, confirmedAt: { type: "string", format: "date-time" } } } } }, confirmationRequired: true }),
...
export async function executeTool(name: string, rawArgs: unknown, context: ToolContext): Promise<ToolResult> {
  ...
  return toolResultSchema.parse({
    ...
    nextTool: name === "load_context" ? "load_project" : name === "list_context_packs" ? "get_context_pack" : name === "start_resource_activity" ? "complete_resource_activity" : undefined,
  });
}
